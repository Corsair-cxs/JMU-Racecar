...
public:
    ...
    tf::StampedTransform stfBaseFootprintWrtOdom_; //base link w/rt odom frame; get this from tf; 
    tf::StampedTransform stfOdomWrtMap_; //odom frame w/rt map frame; get this from tf, published by amcl
    tf::StampedTransform stfBaseFootprint_wrt_Map_; //base link w/rt map frame; compute this
                                              // and publish it on tf
    tf::StampedTransform stfAmclBaseFootprintWrtMap_;  
...
private:
    ...
    ros::Publisher pose_stamped_publisher_; // = nh.advertise<geometry_msgs::PoseStamped>("triad_display_pose", 1, true);
    ros::Publisher amcl_est_base_pub_;
    ...
    nav_msgs::Odometry amcl_est_odom_;
