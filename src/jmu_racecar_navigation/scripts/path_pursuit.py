...
class following_path:
    def __init__(self):
        # TODO gazebo_odom
        # self.current_pose = rospy.Subscriber('/amcl_est_odom', Odometry, self.callback_read_current_position, queue_size=1)
        self.current_pose = rospy.Subscriber('/racecar_wheel_imu_odometry/odom', Odometry, self.callback_read_current_position, queue_size=1)
        # self.current_pose = rospy.Subscriber('/gazebo_module_odom', Odometry, self.callback_read_current_position, queue_size=1)

        self.Pose = []
        self.path_pose = rospy.Subscriber('/move_base/TebLocalPlannerROS/local_plan', Path, self.callback_read_path, queue_size=1)
        self.path_info = []
        self.Goal = []
        self.navigation_input = rospy.Publisher('/racecar/ackermann_cmd', AckermannDriveStamped, queue_size=1)
        self.reach_goal = False
        self.MAX_VELOCITY = 2.2
        # self.MAX_VELOCITY = 1.8
        self.MIN_VELOCITY = 0.2
        self.racecar_D_WHEELS_M = 0.335
        self.max_angle = 0.87
        # self.max_angle = 0.7
        # self.max_angle = 0.75
        # TODO
        # self.steering_angle_velocity = 1.30
        # self.acceleration = 4.40
        # self.jerk = 8.0
        self.steering_angle_velocity = 0.00
        self.jerk = 0.00
    ...
    def callback_read_current_position(self, data):
        ...
        if not len(self.path_info) == 0:
            ...
            print(self.dist(self.Goal, self.Pose))
            ...
            # TODO
            goal_array = np.where((dist_array < (self.LOOKAHEAD_DISTANCE + 0.3)) & (dist_array > 0))[0]
            ...
            r = Goal_dist_/(2*math.sin(diff_angle)) # Calculate the turning radius
            # TODO
            angle = 1.00 * math.atan(self.racecar_D_WHEELS_M/r) # Find the wheel turning radius
            # angle = 2 * math.atan(self.racecar_D_WHEELS_M/r) # Find the wheel turning radius
            ...
            ackermann_control.drive.steering_angle_velocity = self.steering_angle_velocity   
            ackermann_control.drive.jerk = self.jerk   
        else:
            ackermann_control = AckermannDriveStamped()
            ackermann_control.drive.speed = 0.0
            ackermann_control.drive.steering_angle = 0.0
